...
> 💡 **字体精简建议**：如果你安装的 CAD 字体超过 100 个，建议先清理 CAD 安装目录下 `Fonts` 文件夹中的 SHX 字体（保留 `sas_____.pfb`、`MstnFontConfig.xml`、`internat.rsc`、`font.rsc` 等非 SHX 文件），再将本项目提供的 CAD 字体包解压拷贝进去。该字体包在 CAD 原版基础上仅增加了几款常用通用字体（如探索者等），可放心使用。
...
> 💡 `AFRLOG` 每次打开都会重新读取图纸中的实际字体状态，即使通过 `ST`（样式管理器）命令修改过也会正确显示。
